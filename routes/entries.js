...
const calculateRevenue = (wasteType, volume) => {
    const rates = {
        ...
        bilge: 110,
        hazardous: 250,
        "n/a": 250  // FIXED: Added support for 'N/A' labeling
    };
    const typeKey = wasteType?.toLowerCase() || 'other';
    const rate = rates[typeKey] || 25; 
 ...
 * @desc    Get single entry by ID (Required for Edit Form)
 */
 ...
 * @desc    Update vessel entry and recalculate revenue
 */
...
router.get('/search', auth, async (req, res) => {
    try {
        ...
        const entries = await Entry.find(query).sort({ createdAt: -1 }).lean();
        res.json(entries);
    } catch (err) {
